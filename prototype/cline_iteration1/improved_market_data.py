...
class MarketDataManager:
    ...
    def plot_stock_data(self, save_path=None, show_plot=False):
        """
        ...
        logger.info(f"Plotting data for {self.ticker_symbol}")
        ...
        try:
            ...
            if 'BB_High' in self.data.columns:
                ax1.plot(self.data.index, self.data['BB_High'], 'r--', label='Bollinger High', alpha=0.5)
                ax1.plot(self.data.index, self.data['BB_Low'], 'r--', label='Bollinger Low', alpha=0.5)
            
            ax1.set_title(f'{self.ticker_symbol} Stock Price and Indicators')
            ...
            if 'MACD' in self.data.columns and 'MACD_Signal' in self.data.columns:
                ...
                ax2.plot(self.data.index, self.data['MACD_Signal'], label='Signal', color='red')
                
                # Plot histogram
                if 'MACD_Hist' in self.data.columns:
                    ax2.bar(self.data.index, self.data['MACD_Hist'], label='Histogram', 
                           color=self.data['MACD_Hist'].apply(lambda x: 'green' if x > 0 else 'red'),
                           alpha=0.5, width=5)
            ...
            if save_path is None:
                save_path = f"{self.ticker_symbol}_stock_report.png"
            ...
            logger.error(f"Error plotting stock data: {e}")
            raise RuntimeError(f"Failed to plot stock data: {str(e)}")
    ...
    def generate_report(self, prompt_template=None):
        ...
        logger.info(f"Generating report for {self.ticker_symbol}")
        ...
        if prompt_template is None:
            prompt_template = (
                "You are a TV anchor in financial news. You are knowledgeable about pricing, "
                "trends, and the market. Give me an insightful report for {ticker} as if you "
                "really care about this company's past, present and future. "
                "The current price is ${latest_price:.2f}. "
                "The 52-week high is ${year_high:.2f} and the low is ${year_low:.2f}. "
